        ...
        if (canAttack)
        {
            if (!canPressButton)
            {
                if(canDamage)
                {
                    ...
                    if(hit5.transform.GetComponent<SlimeScript>().slime == SlimeScript.slimeType.Minion)
                    {
                        GetComponent<SlimeBoss>().enemiesDestroyed += 1;
                    }

                    /*if (hit5.transform.gameObject.GetComponent<SlimeBoss>().transform)
                    {
                        if (hit5.transform.gameObject.GetComponent<SlimeBoss>().bossCanBeDamaged == true)
                        {
                            hit5.transform.gameObject.GetComponent<SlimeBoss>().health -= 1;
                            hit5.transform.gameObject.GetComponent<SlimeBoss>().hit = true;
                        }
                    }
                    if (hit5.transform.gameObject.GetComponent<SlimeScript>().transform)
                    {
                        hit5.transform.gameObject.GetComponent<SlimeScript>().health -= 1;
                        hit5.transform.gameObject.GetComponent<SlimeScript>().hit = true;
                    }*/
